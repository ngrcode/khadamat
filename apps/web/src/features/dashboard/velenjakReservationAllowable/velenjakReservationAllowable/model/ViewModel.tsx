...
import { QUERY_VELENJAKRESERVATIONALLOWABLE } from '@/constants/endPoint/velenjakreservationallowable';
import { getVelenjakTotalCount, mapVelenjakItems } from '../mapper';

export const DATE_FIELDS = [];

export const getInitialValues = (): VelenjakReservationAllowableSearchParams => ({});
...
const DEFAULT_COLUMNS = [
  ...
  { data: 'nationality', name: '', searchable: true, orderable: true },
  { data: 'mobile', name: '', searchable: true, orderable: true },
  { data: 'email', name: '', searchable: true, orderable: true },
  { data: 'unit_name', name: '', searchable: true, orderable: true },
  { data: 'velenjakReservation', name: '', searchable: true, orderable: true },
  { data: 'barberShop', name: '', searchable: true, orderable: true },
...
export const buildQueryKey = (
  search: VelenjakReservationAllowableSearchParams,
  pagination: { pageIndex: number; pageSize: number },
  extra?: any[]
) => {
  return [
    QUERY_VELENJAKRESERVATIONALLOWABLE,
    search,
...
export const formatSearchDates = (
  search: VelenjakReservationAllowableSearchParams
): VelenjakReservationAllowableSearchParams => formatDateFields(search, DATE_FIELDS);
...
export const selectData = (response: any, pageSize: number = 10) => {
  ...
  const items = mapVelenjakItems(response);
  const totalCount = getVelenjakTotalCount(response, items);
