...
import { QUERY_UNITEMPLOYEE } from '@/constants/endPoint/unitemployee';
import { getUnitEmployeeTotalCount, mapUnitEmployeeItems } from '../mapper';

export const DATE_FIELDS = [];

export const getInitialValues = (): UnitEmployeeSearchParams => ({});
...
export const buildQueryKey = (
  search: UnitEmployeeSearchParams,
  pagination: { pageIndex: number; pageSize: number },
  extra?: any[]
) => {
  return [
...
export const formatSearchDates = (
  search: UnitEmployeeSearchParams
): UnitEmployeeSearchParams => formatDateFields(search, DATE_FIELDS);

export const selectData = (response: any, pageSize: number = 10) => {
  const items = mapUnitEmployeeItems(response);
  const totalCount = getUnitEmployeeTotalCount(response, items);
