...
import { QUERY_EXSEL } from '@/constants/endPoint/exsel';
import { getExcelTotalCount, mapExcelItems } from '../mapper';

export const DATE_FIELDS = [];

export const getInitialValues = (): ExcelSearchParams => ({});
...
const DEFAULT_COLUMNS = [
  ...
  { data: 'month', name: '', searchable: true, orderable: true },
...
export const formatSearchDates = (
  search: ExcelSearchParams
): ExcelSearchParams => formatDateFields(search, DATE_FIELDS);

export const selectData = (response: any, pageSize: number = 10) => {
  const items = mapExcelItems(response);
  const totalCount = getExcelTotalCount(response, items);
