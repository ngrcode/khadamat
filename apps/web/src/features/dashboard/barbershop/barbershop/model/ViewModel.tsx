...
import { BarbershopSearchParams } from '../types';
import { QUERY_BARBERSHOP } from '@/constants/endPoint/barbershop';
import { getBarbershopTotalCount, mapBarbershopItems } from '../mapper';

export const DATE_FIELDS = [];

export const getInitialValues = (): BarbershopSearchParams => ({});
...
export const formatSearchDates = (
  search: BarbershopSearchParams
): BarbershopSearchParams => formatDateFields(search, DATE_FIELDS);
...
export const selectData = (response: any, pageSize: number = 10) => {
  ...
  const items = mapBarbershopItems(response);
  const totalCount = getBarbershopTotalCount(response, items);
