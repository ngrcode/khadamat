...
import { GET_BARBERSHOP_URL, QUERY_BARBERSHOP } from '@/constants/endPoint/barbershop';
import { employeeBarberShopAllowable } from '@/generated/api/employee/employee';
import type { Commons } from '@/generated/api/model';

const BarbershopReport = () => {
  ...
  const handleDelete = useCallback(async (record: any) => {
    try {
      ...
      throw new Error(`Delete is not available for barbershop record ${id}`);
    } catch (error) {
  ...
  const vm = useReportViewModel<BarbershopSearchParams>({
    url: GET_BARBERSHOP_URL,
    getInitialValues,
    getValidationSchema,
    buildQueryParams,
    buildQueryKey,
    ...
    selectData,
    getColumns,
    requestFn: (params) => employeeBarberShopAllowable(params as Commons, { headers }),
    rowKey: 'id',
    fetchOnMount: true,
    method: 'post',
    headers,
  ...
  return (
    <ReportPage<BarbershopSearchParams>
      vm={vm}
      userType={userType}
      filterFields={false}
      ...
      config={{
        ...
        title: t('barbershop'),
        showDownload: false,
        subtitle: t('barbershopReportSubtitle'),
        rowKey: 'id',
        scrollX: 2100,
        settingsKey: QUERY_BARBERSHOP,
        enableRowDrag: true,
...
export default BarbershopReport;
