...
import { DELETE_NOTIFICATION_URL, GET_NOTIFICATION_URL, QUERY_NOTIFICATION } from '@/constants/endPoint/notification';
import { notificationDelete, notificationIndex } from '@/generated/api/notification/notification';

const NotificationlReport = () => {
  ...
  const handleDelete = useCallback(async (record: any) => {
    try {
      ...
      await notificationDelete({ Id: Number(id) }, { headers });
  ...
  const vm = useReportViewModel<WageMonthlySearchParams>({
    ...
    selectData,
    getColumns,
    requestFn: () => notificationIndex({ headers }),
    rowKey: 'id',
    fetchOnMount: true,
    method: 'post',
    headers,
  ...
  return (
    <ReportPage<WageMonthlySearchParams>
      ...
      config={{
        ...
        showDownload: false,
        subtitle: t('notificationReportSubtitle'),
        rowKey: 'id',
        scrollX: 1400,
        settingsKey: QUERY_NOTIFICATION,
        enableRowDrag: true,
...
export default NotificationlReport;
